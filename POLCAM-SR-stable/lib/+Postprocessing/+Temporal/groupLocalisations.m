...
for id_seed = 1:numel(idx_seed) % append grouped localisations to 'dataGrouped'
    ...
    if ismember('x_beforeDriftCorrection', dataLinked.Properties.VariableNames)
        dataGrouped_i.x_beforeDriftCorrection = mean(toGroup.x_beforeDriftCorrection);
    end
    if ismember('y_beforeDriftCorrection', dataLinked.Properties.VariableNames)
        dataGrouped_i.y_beforeDriftCorrection = mean(toGroup.y_beforeDriftCorrection);
    end
